...
export default function AdminDashboard() {
  ...
  // Don't redirect - allow demo access even if session fetch fails
  // The page will still work with the data that loads
        ...
        <div className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <Card className="card-glow border-border/60 bg-card/80 backdrop-blur-sm">
            <CardContent className="flex items-center gap-4 py-5">
              ...
              <div>
                <p className="text-2xl font-bold text-foreground">{stats?.activeTests ?? 0}</p>
          ...
          <Card className="card-glow border-border/60 bg-card/80 backdrop-blur-sm">
            <CardContent className="flex items-center gap-4 py-5">
              ...
              <div>
                <p className="text-2xl font-bold text-foreground">{stats?.eligible ?? 0}</p>
          ...
          <Card className="card-glow border-border/60 bg-card/80 backdrop-blur-sm">
            <CardContent className="flex items-center gap-4 py-5">
              ...
              <div>
                <p className="text-2xl font-bold text-foreground">{stats?.totalViolations ?? 0}</p>
            ...
            <CardContent>
              <div className="grid gap-4 sm:grid-cols-3">
                <div className="rounded-lg border border-accent/20 bg-accent/5 p-4">
                  ...
                  <Progress
                    value={stats?.totalStudents ? ((stats.eligible ?? 0) / stats.totalStudents) * 100 : 0}
                ...
                <div className="rounded-lg border border-yellow-500/20 bg-yellow-500/5 p-4">
                  ...
                  <Progress
                    value={stats?.totalStudents ? ((stats.borderline ?? 0) / stats.totalStudents) * 100 : 0}
                    className="mt-2 h-1.5"
                  />
                </div>
                <div className="rounded-lg border border-destructive/20 bg-destructive/5 p-4">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-foreground">Not Eligible</span>
                  </div>
                  <p className="mt-2 text-3xl font-bold text-destructive">{stats?.notEligible ?? 0}</p>
                  <Progress
                    value={stats?.totalStudents ? ((stats.notEligible ?? 0) / stats.totalStudents) * 100 : 0}
                    className="mt-2 h-1.5"
                  />
                </div>
              </div>
                  ...
                  <TableBody>
                    {filteredStudents.map((student) => (
                      <TableRow key={student.id}>
                        ...
                        <TableCell>
                          <div>
                            <p className="text-sm text-foreground">{student.registrationId}</p>
                        ...
                        <TableCell className="text-center">
                          <span className="text-sm text-foreground">
                            {student.currentAttempt}/2
                          </span>
                        </TableCell>
                        <TableCell className="text-center">
                          <span
                            className={`text-sm font-medium ${
                              student.violations >= 6
                                ? "text-destructive"
                          ...
                          <div className="flex items-center justify-end gap-2">
                            {student.status === "completed" && (
                              <Button
                                ...
                                disabled={evaluatingId === student.id}
                              >
                                {evaluatingId === student.id ? (
